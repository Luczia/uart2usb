...
int main(void)
{
  ...
  /*
  // Display a heder frame at startup // You may uncomment this for debug
  uint8_t buffer_header[] = "Demarrage de la passerelle!\r\n";
  CDC_Transmit_HS(buffer_header, sizeof(buffer_header));
  HAL_UART_Transmit_IT(&huart7, buffer_header, sizeof (buffer_header));
  */
...
/*	Send debug data on UART and USB  // You may uncomment this for debug
	  uint8_t buffer2[] = "Test\n";
	  CDC_Transmit_HS(buffer2, sizeof(buffer2));//
	  uint8_t data[] = "H W\n";
	  HAL_UART_Transmit_IT(&huart7, data, sizeof (data)); */
